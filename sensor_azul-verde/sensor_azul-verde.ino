...
int valorDedoEstirado = 2400; //2390
int valorDedoDoblado = 2510;  
...
void loop() {
  ...
  // MODIFICACIÓN 2: Anulamos la inercia del filtro si el valor baja 
  // para que la caída sea completamente inmediata.
  if (valorBrutoPromedio < valorFiltrado) {
    valorFiltrado = valorBrutoPromedio; 
  } 

  // === NUEVA LÓGICA AGREGADA: Filtro de suelo estático ===
  // Si el sensor está en reposo o el ruido residual intenta subir menos de un 25% del rango útil,
  // congelamos el filtro en el valor estirado para que no acumule inercia hacia arriba.
  else if (valorBrutoPromedio <= (valorDedoEstirado + 30)) { //30
    valorFiltrado = valorDedoEstirado;
  }
